...
export const generateColors: () => TileColor[][] = () => {
  ...
  const colors: TileColor[][] = [[white, white, white, white, white], [white, white, white, white, white], [white, white, white, white, white], [white, white, white, white, white], [white, white, white, white, white]];
  let redCount = 8;
  let blueCount = 8;
  let blackCount = 1;
...
const check = (i: number, j: number, c?: [number, number]) => {
  if (!c) {
    return false;
  }
  return (c[0] === i && c[1] === j)
...
function Game() {
  ...
  const [selectedWord, setSelectedWord] = React.useState<[number, number] | null>(null);
  const [focusedWord, setFocusedWord] = React.useState<[number, number] | null>(null);
  return (
    <div>
      ...
      <Box sx={{ flexGrow: 1, width: 800, height: 600 }}>
            ...
            tileMap.map((row, i) => {
              return (
                <Stack direction="row" spacing={2}>
                  {
                    row.map((word, j) => {
                      let isSelected = check(i, j, selectedWord!);
                      let isFocused = check(i, j, focusedWord!);
                      return (
                        <Item sx={{ width: 200, backgroundColor: toColor(word, isSelected, isFocused) }} onMouseEnter={
                          () => {
        ...
        {selectedWord &&
          <div>
            <div>Seçilen Kelime: {tileMap[selectedWord[0]][selectedWord[1]].word}</div>
            <Button onClick={() => {
...

        {
          <div>
            ...
            {
              tileMap.map((row) => {
                return row.filter((tile) => tile.color === red && tile.state === "opened").length;
              }).reduce((a, b) => a + b)
            }
            /
            {
...

        {
          <div>
            ...
            {
              tileMap.map((row) => {
                return row.filter((tile) => tile.color === blue && tile.state === "opened").length;
              }).reduce((a, b) => a + b)
            }
            /
            {
